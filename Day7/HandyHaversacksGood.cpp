    ...
    for (unsigned i = 0; i < currentMapPath.size(); i++) {
        ...
        if (rHSMap.count(currentMapPath[i]) > 0) {
            ...
            for (unsigned j = lastPosOfTreePath; j >= 1; j--) {
                ...
                if (node->getChildren().count(createTreePath[j]) == 0) {
                    node->addNodeToChildren(new TrieNode(createTreePath[j]), createTreePath[j]);
                }
...
int main() {
    std::string line;
    std::ifstream myFile("HandyHaversacks2.txt");
    if (myFile.is_open()) {
        ...
        for (auto& element : rightHandSideMap) {
            for (auto& elementValueColor : element.second) {
                std::vector<std::string> startVector;
                startVector.emplace_back(elementValueColor);
                if (element.first == "shiny gold") {
                    updateTree(root, root, rightHandSideMap, outermostBagColorsCountedAlready, startVector, {}, true, returnCounter);    
                }
                else {
                    updateTree(root, root, rightHandSideMap, outermostBagColorsCountedAlready, startVector, {}, false, returnCounter); 
                }
            }
        }
